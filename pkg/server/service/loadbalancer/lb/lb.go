...
func (b *LBBalancer) Add(name string, handler http.Handler, burst *int, average *int, period *int, priority *int) {
	...
	bucket := rate.NewLimiter(rate.Every((time.Second*time.Duration(p))/time.Duration(a)), bu)
	canAllow := true
	h := &namedHandler{Handler: handler, name: name, burst: int64(bu), average: int64(a), period: time.Second * time.Duration(p), priority: int64(prio), bucket: bucket, canAllow: canAllow}
